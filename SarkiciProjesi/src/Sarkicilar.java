...
public class Sarkicilar {
    ...
    public void Sarkicilari_bastir(){
        System.out.println("Şarkıcı Listesinde "+sarkici_listesi.size() + " kadar şarkı vardır.");
    ...
    public void sarkici_sil(int pozisyon){
        ...
        sarkici_listesi.remove(isim);
        System.out.println(isim+" listeden silindi...");
    }
    public void sarkici_ara(String isim){
        int pozisyon = sarkici_listesi.indexOf(isim);
        if(pozisyon>=0){
            System.out.println("Şarkıcı Bulundu...");
            System.out.println(isim+" isimli şarkıcı "+(pozisyon+1)+" pozisyonunda...");
        }
        else
